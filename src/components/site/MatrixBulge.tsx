    ...
    <div className={`relative w-full overflow-hidden flex items-center gap-[3px] md:gap-[4px] ${className}`} aria-hidden>
      {bars.map((b, i) => (
        <div
          ...
          style={{
            background: b.color,
            // Base height leaves room for the scale-up to be visible.
            height: "45%",
            // Stagger delay across the row so the scale-up wave visibly
            // travels from left to right, repeating on loop.
