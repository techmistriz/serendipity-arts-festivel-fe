...
export function CartProvider({ children }: { children: ReactNode }) {
  const [items, setItems] = useState<CartItem[]>(() => getStoredValue(CART_KEY, []));
  const [bookings, setBookings] = useState<CartItem[]>(() => getStoredValue(BOOKINGS_KEY, []));
  const [isVip] = useState(() => getStoredValue<string>(VIP_KEY, "") === "1");
  const [wishlist, setWishlist] = useState<string[]>(() => getStoredValue(WISH_KEY, []));
