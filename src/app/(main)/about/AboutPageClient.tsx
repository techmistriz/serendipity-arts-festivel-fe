...
export function AboutPageClient() {
  ...
  useEffect(() => {
    ...
    const loadLaunchFilms = async () => {
      try {
        const films = await getLaunchFilms();

        if (isCurrent) setLaunchFilms(films);
      } catch {
        if (isCurrent) setFilmsError("Unable to load launch films.");
      } finally {
        if (isCurrent) setLoadingFilms(false);
      }
    };

    void loadLaunchFilms();
  ...
  const retryLaunchFilms = async () => {
    try {
      setLoadingFilms(true);
      setFilmsError(null);
      setLaunchFilms(await getLaunchFilms());
    } catch {
