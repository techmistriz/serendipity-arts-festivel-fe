  ...
  useEffect(() => {
    async function loadVenues() {
      try {
        ...
        const visibleVenues = data.filter((venue) => venue.is_hide_on_frontend !== 1);
  ...
  return (
    <div className="container-editorial pt-10 md:pt-20 pb-32">
      ...
      <p className="mt-6 max-w-3xl text-muted-foreground">
        ...
        workshops, and more. Spread across Panjim’s riverfront, the Festival transforms the city
        into a buzzing cultural hub — accessible to all.
      </p>
      {/* Loading */}
      {/* {loading && (
          ...
          {venues
            ...
            .map((venue) => (
              ...
              >
                ...
                <p className="mt-2 text-sm text-muted-foreground line-clamp-2 headline">
                  {" "}
    ...
    <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-sm overflow-y-auto ed-fade">
      ...
      <div className="container-editorial pt-6 md:pt-10 pb-16">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10">
          ...
          <div className="md:col-span-6">
            <h2 className="display uppercase text-3xl md:text-6xl leading-[0.92] tracking-[-0.02em]">
              {venue.title}
            </h2>
            <div
              className="mt-4 text-base md:text-lg max-w-prose leading-relaxed headline whitespace-pre-line"
          ...
          <div className="mt-16 md:mt-20 rule-t pt-8">
            <div className="flex items-baseline justify-between">
              ...
              <p className="label text-muted-foreground">
                {venue.program_details.length} programmes
              </p>
            </div>
            <ul className="mt-6 divide-y divide-rule">
              {venue.program_details.map((item) => (
                <li key={item.id}>
                  ...
                  >
                    <div>
                      <p className="headline font-semibold text-base md:text-xl leading-tight">
                        {" "}
                        {item.program?.name}
                      </p>
                      <p className="label text-muted-foreground mt-1 group-hover:text-accent transition-colors">
                        ...
                        {item.program?.curators?.length > 0 && (
                          <>
                            {" "}
                            · Curated by{" "}
                            {item.program.curators.map((curator) => curator.name).join(", ")}
                          </>
                        )}
                      </p>
                    </div>
                    <p className="label text-muted-foreground shrink-0 text-right group-hover:text-accent transition-colors">
                      {" "}
                      {item.event_date}
                      <br />
