...
import { categoryStyle, tagStyle, priceStyle } from "@/src/lib/tag-colors";
...
import {
    ...
    relatedProgrammes,
    programmeById,
} from "@/src/data/programmes-data";
import { RecommendModal } from "@/src/components/common/RecommendModal";
                ...
                <nav className="mt-14 flex flex-wrap items-center justify-center gap-2" aria-label="Pagination">
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
                        <button
                            ...
                            className={`headline text-xs uppercase tracking-[0.06em] min-w-[40px] px-3 py-2 border ${n === currentPage
            ...
            <div className="container-editorial pt-6 md:pt-10 pb-16">
                ...
                <div className="mt-8 md:mt-10 grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10">
                    <div className="md:col-span-6">
                        <Image src={programme.img} alt={programme.title} className="w-full aspect-square object-cover" />
                    </div>
                            ...
                            <div className="mt-3 mb-6 flex flex-wrap gap-2">
                                {programme.slots.map((s, i) => (
                                    <button
                                        ...
                                        className={`headline text-xs uppercase tracking-[0.06em] border px-3 py-2 transition-colors ${slotIdx === i
                                            ? "bg-foreground text-background border-foreground"
                                            : "border-foreground hover:bg-foreground hover:text-background"
                                            }`}
                                    >
                                        ...
                                        {programme.includes.map((inc) => {
                                            ...
                                            return (
                                                <li key={inc.title} className="py-2">
                                                    ...
                                                    >
                                                        {ref && (
                                                            <Image src={ref.img} alt="" className="w-14 h-14 object-cover border border-foreground shrink-0" />
                                                        )}
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="mt-3 flex flex-wrap items-center gap-3">
                                                            ...
                                                            <button
                                                                ...
                                                                aria-pressed={on}
                                                                className={`headline text-xs uppercase tracking-[0.08em] border border-foreground px-5 py-2.5 transition-colors ${on ? "bg-foreground text-background" : "hover:bg-foreground hover:text-background"
                                                                    }`}
                                                            >
            ...
            {showTerms && (
                <SidePanel onClose={() => setShowTerms(false)} label="Disclaimer">
                    ...
                    <p className="mt-4 text-sm text-muted-foreground headline">
                        Read the full <Link href="/terms" className="text-foreground underline underline-offset-4">Terms &amp; Conditions</Link>.
                    </p>
                </SidePanel>
            )}
            ...
            {clashItem && (
                <Modal onClose={() => setClashItem(null)} label="Time clash">
                    ...
                    <div className="mt-8 flex flex-wrap gap-3">
                        ...
                        <Link href="/cart" className="headline text-xs uppercase tracking-[0.06em] border border-foreground px-5 py-3 hover:bg-foreground hover:text-background transition-colors">
                            Go to cart →
                        </Link>
                    </div>
                </Modal>
...
export default function ProgrammesList({
    ...
    return (
        <Suspense fallback={null}>
            <ProgrammesListContent initialCategory={initialCategory} />
        </Suspense>
    );
}