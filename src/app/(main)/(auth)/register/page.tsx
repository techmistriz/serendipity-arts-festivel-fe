...
function RegisterContent() {
    ...
    const {
        ...
        archivedUser,
        archiveError,
    } = useCheckArchiveUser(email, roleId);
    ...
    useEffect(() => {
        ...
        console.log("FILLING ARCHIVED USER:", archivedUser);
        ...
        setValue(
            "std_code",
            archivedUser.std_code ?? "91"
        );

        setValue(
            "whatsapp",
            archivedUser.contact
                ? String(archivedUser.contact)
                : ""
        );

        setValue(
            "country",
            archivedUser.country_id
                ? String(archivedUser.country_id)
                : ""
        );

        setValue(
            "state",
            archivedUser.state_id
                ? String(archivedUser.state_id)
                : ""
        );

        setValue(
            "city",
            archivedUser.city_id
                ? String(archivedUser.city_id)
                : ""
        );
        ...
        setValue(
            "age",
            archivedUser.age_group ?? ""
        );

        setValue(
            "visitedYears",
            archivedUser.visited_year ?? []
        );

        setValue(
            "newsletter",
            archivedUser.subscribe === 1
        );
    ...
    const onSubmit = async (data: any) => {
        ...
        try {
            ...
            if (isSea) {
                ...
                console.log("General Registration Payload:", generalData);
                console.log("userExists:", userExists);
                console.log("is_old_user:", userExists === true ? 1 : 0);
                console.log("Payload:", generalData);
            ...
            <div className="relative overflow-hidden bg-black text-white aspect-[16/7] md:aspect-[16/5] w-full">
                ...
                <div className="absolute inset-0 flex items-center justify-center p-6 md:p-10">
                    <h1 className="display uppercase text-white text-[9vw] md:text-[5.5vw] leading-[0.9] tracking-[-0.03em] text-center max-w-[16ch]">
                        {isSea ? "SEA Registration" : isGuest ? "Special Guest Registration" : "Visitor Registration"}
                    </h1>
                </div>
            </div>
