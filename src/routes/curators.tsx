...
import { CURATORS, type Curator } from "@/lib/curators";
import { programmesByCurator, dateLabel } from "@/lib/programmes-data";
import { GlitchBar } from "@/components/site/GlitchBar";
import { GlitchBorder } from "@/components/site/GlitchBorder";


export const Route = createFileRoute("/curators")({
  head: () => ({
    meta: [
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10">
              <div className="md:col-span-5">
                <GlitchBorder seed={active.name.length + 23} thickness={1} hoverBoost={14} delayMs={200} className="overflow-hidden">
                  <img src={active.img} alt={active.name} className="w-full aspect-[4/5] object-cover" />
                </GlitchBorder>
              </div>
