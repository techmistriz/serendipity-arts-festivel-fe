...
import { BookingSuccess } from "@/components/site/BookingSuccess";

export const Route = createFileRoute("/cart")({
  head: () => ({
    meta: [
      { title: "Cart — Serendipity Arts Festival 2026" },
      { name: "description", content: "Your booked programmes for Serendipity Arts Festival 2026." },
    ],
  }),
  component: CartPage,
});
...
function CartPage() {
  const { items, remove, setQty, subtotal, isRegistered, isLoggedIn, confirmBooking, isVip } = useCart();
  const navigate = useNavigate();
  const [showSuccess, setShowSuccess] = useState(false);

  const checkout = () => {
    if (!isRegistered) {
      navigate({ to: "/register", search: { next: "/cart" } as never });
      return;
    }
    if (!isLoggedIn) {
      navigate({ to: "/login", search: { next: "/cart" } as never });
      return;
    }
    setShowSuccess(true);
  };
  ...
  if (items.length === 0 && !showSuccess) {
    return (
      <div className="container-editorial pt-12 md:pt-24 pb-32">
        ...
        <Link to="/programmes" className="mt-8 inline-block label border border-foreground px-6 py-3 hover:bg-foreground hover:text-background transition-colors">
          Browse programmes →
        </Link>
      </div>
    );
        ...
        {items.map((it) => (
          <li key={it.id} className="rule-b py-5 md:py-6 grid grid-cols-[64px_1fr_auto] md:grid-cols-[96px_1fr_auto_auto] items-center gap-4 md:gap-6">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              <button onClick={() => setQty(it.id, it.qty - 1)} className="headline font-semibold text-xl w-8">−</button>
              <span className="headline font-semibold text-xl tabular-nums w-6 text-center">{it.qty}</span>
              <button onClick={() => setQty(it.id, Math.min(isVip ? 2 : 6, it.qty + 1))} className="headline font-semibold text-xl w-8">+</button>
            </div>
            <button onClick={() => remove(it.id)} className="label text-muted-foreground hover:text-accent col-span-3 md:col-span-1 justify-self-end">Remove</button>
          </li>
        ))}
      </ul>
