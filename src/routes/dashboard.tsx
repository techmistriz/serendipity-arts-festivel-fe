...
import { PROGRAMMES, dateLabel, timeLabel } from "@/lib/programmes-data";
import { Heart } from "lucide-react";

export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      { title: "Dashboard — Serendipity Arts Festival 2026" },
      { name: "description", content: "Your festival: bookings, wishlist and schedule." },
    ],
  }),
  component: Dashboard,
});
...
// "19:30" → "7:30 PM"; passthrough for ranges already formatted
...
function Dashboard() {
  const { bookings: confirmedBookings, isLoggedIn, wishlist, toggleWish } = useCart();
  const wishlistItems = useMemo(
    () => PROGRAMMES.filter((p) => wishlist.includes(p.id)),
    [wishlist],
  );
  ...
  // Aggregate quantities across sessions for the same programme title
  const bookings = useMemo(() => {
    const byTitle = new Map<string, { id: string; title: string; venue: string; dateSet: Set<number>; time: string; qty: number; price: number }>();
    for (const it of confirmedBookings) {
      const day = parseInt(it.date, 10);
      const prev = byTitle.get(it.title);
      if (prev) {
        prev.qty += it.qty;
        if (!Number.isNaN(day)) prev.dateSet.add(day);
      } else {
        byTitle.set(it.title, {
          id: it.id, title: it.title, venue: it.venue,
          dateSet: new Set(Number.isNaN(day) ? [] : [day]),
          time: fmtAmPm(it.time), qty: it.qty, price: it.price,
        });
      }
    }
    return Array.from(byTitle.values()).map((b) => ({
      ...b, dates: Array.from(b.dateSet).sort((a, z) => a - z),
    }));
  }, [confirmedBookings]);
    ...
    return (
      <div className="container-editorial pt-16 md:pt-24 pb-40">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link to="/login" className="label border border-foreground px-6 py-3 hover:bg-foreground hover:text-background transition-colors">
            Log in →
          </Link>
          <Link to="/register" className="label border border-foreground px-6 py-3 hover:bg-foreground hover:text-background transition-colors">
            Register →
          </Link>
        </div>
      </div>
        ...
        {(["bookings", "wishlist", "schedule", "profile"] as const).map((t) => (
          <button
            ...
            className={`display uppercase text-2xl md:text-3xl leading-none transition-colors relative pb-1 ${
              tab === t ? "text-foreground pink-underline" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {t}
          </button>
      ...
      <div className="mt-10">
        {tab === "bookings" && (
          bookings.length === 0 ? (
            <p className="label text-muted-foreground py-16 text-center">
              No bookings yet. Browse <Link to="/programmes" className="text-foreground underline underline-offset-4">programmes</Link>.
            </p>
          ) : (
            <ul className="rule-t">
              {bookings.map((b) => (
                <li key={b.id} className="rule-b py-6 grid grid-cols-12 gap-4 items-baseline">
                  ...
                  <div className="col-span-6 md:col-span-5">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">{b.time} · {b.venue}</p>
                  </div>
        ...
        {tab === "wishlist" && (
          wishlistItems.length === 0 ? (
            <p className="label text-muted-foreground py-16 text-center">
              Nothing saved yet. Browse <Link to="/programmes" className="text-foreground underline underline-offset-4">programmes</Link> and tap the heart to save.
            </p>
          ) : (
            <ul className="rule-t">
              {wishlistItems.map((p) => (
                <li key={p.id} className="rule-b py-6 grid grid-cols-12 gap-4 items-center">
                  <img src={p.img} alt={p.title} className="col-span-2 md:col-span-1 w-full aspect-square object-cover" />
                  <div className="col-span-7 md:col-span-8">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground headline">{dateLabel(p)} · {timeLabel(p)} · {p.venue}</p>
                  </div>
                  <div className="col-span-3 flex flex-wrap items-center justify-end gap-2">
                    <Link to="/programmes" search={{ p: p.id } as never} className="label border border-foreground px-3 py-2 hover:bg-foreground hover:text-background transition-colors">
                      Book →
                    </Link>
                    <button onClick={() => toggleWish(p.id)} aria-label="Remove from wishlist" className="p-2 hover:text-accent">
                  ...
                  <ul className="mt-4 space-y-3">
                    ...
                    {dayItems.map((b) => (
                      <li key={b.id + d} className="text-xs border-l-2 border-accent pl-2">
                        <p className="label text-accent tabular-nums">{b.time}</p>
